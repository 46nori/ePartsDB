...
- **内容**:
  ...
  - リリース済みの機能
  - 本番環境対応済みのコード
- **GitHub Actions**: lint・テスト・ビルドのみ（デプロイなし）
- **使用者**: エンドユーザー・本番環境
...
- **内容**:
  ...
  - 個人の実際のパーツDB
  - 運用固有の設定
- **GitHub Actions**: lint・テスト・ビルド + GitHub Pagesデプロイ
- **使用者**: エンドユーザー（個人）
...
npm run sync:db  # 必要に応じてgh-pagesの最新DBを同期
# ... 開発作業 ...
...
- **CI**: 全ブランチで lint・テスト・ビルドを実行
- **デプロイ**: `gh-pages`ブランチのみ

### CIステップ（build-and-deploy）
...
4. `npm test`
5. `npm run build`
6. （`gh-pages`のみ）GitHub Pagesへデプロイ
...
- **早期エラー検出**: 開発段階でのlint・ビルドエラーを即座に発見
- **DB互換性検証**: gh-pagesの`eparts.db`との同期テストを実行
...
- **`develop` ブランチ**: lint・テスト・ビルドのみ（約1分）
- **`main` ブランチ**: lint・テスト・ビルドのみ（約1分）
- **`gh-pages` ブランチ**: lint・テスト・ビルド + デプロイ（約2〜4分）
...
4. **品質保証**: lint・DBテスト・ビルドによる段階的検証
5. **安全性**: 個人データがdevelop/mainブランチに混入しない（`sync:db`は任意）
