...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showPartsByCategory(categoryId, categoryName) {
    ...
    try {
      ...
      // パーツ一覧のHTML生成（新しい列順序: 在庫数、部品名、種別、型番、外形、説明）
      let partsHtml = `
      ...
      if (parts.length === 0) {
        ...
        parts.forEach(part => {
          partsHtml += `
            <tr>
              <td>${formatStockQuantity(part.quantity)}</td>
              ...
              ${isLocalEnvironment ? `
                <td class="actions">
                  ...
                  <button class="btn-inventory" data-part-id="${part.id}" title="在庫変更">📦</button>
  ...
  function doSearch() {
    ...
    try {
      ...
      // 検索結果のHTML生成（新しい列順序: 在庫数、部品名、種別、型番、外形、説明、カテゴリ）
      let resultsHtml = '';
  ...
  function setupRowActionEvents(containerId) {
    ...
    container.querySelectorAll('.btn-inventory').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // ダイアログ関数が読み込まれているかチェック
        if (typeof window.showInventoryDialog === 'function') {
          window.showInventoryDialog(partId);
        } else {
          console.error('showInventoryDialog関数が見つかりません');
          alert('在庫変更機能が読み込まれていません。ページを再読み込みしてください。');
        }
      });
    });
